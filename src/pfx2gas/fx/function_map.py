...
FUNCTION_MAP: dict[str, FnSpec] = {
    ...
    "IsBlankOrError": FnSpec("FX.isBlankOrError({a0})", "special-cased deferred evaluation"),
    ...
    "Find": FnSpec("FX.find({args})"),
    "IsMatch": FnSpec("FX.isMatch({args})", "constant JavaScript-compatible canvas pattern"),
    "Match": FnSpec("FX.match({args})", "constant canvas pattern; record captures"),
    "MatchAll": FnSpec("FX.matchAll({args})", "constant canvas pattern; table of captures"),
    "Proper": FnSpec("FX.proper({a0})"),
    "Substitute": FnSpec("FX.substitute({a0}, {a1}, {a2})"),
    "Replace": FnSpec("FX.replace({a0}, {a1}, {a2}, {a3})"),
    "Text": FnSpec("FX.text({args})"),
    ...
    "TimeValue": FnSpec("FX.timeValue({args})", "clock text, localized day periods and ISO timestamps"),
    # tables
