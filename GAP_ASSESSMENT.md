...
|---|---|---|
| Unit/runtime tests | 392 Python pass, 3 skip; 93 JS pass; bare globals, FX, FXRuntime and FX.collections emitter/runtime consistency passes | Complete deployed workflows and broader control semantics |
...
| Horizontal loading galleries | Diagnostic native-directory and pre-directory-contract runs both grow Employee Ideas' page to 33,554,432 pixels; full-page capture crashes and no People request occurs | TemplateWidth incorrectly uses the whole gallery width. Preserve orientation/template geometry before claiming usable initialization failure handling |
